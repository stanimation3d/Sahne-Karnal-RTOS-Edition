...
pub struct HardwareConfig {
    /// Seri Port (UART) MMIO adresi (Genellikle KSEG1 bölgesindedir)
    pub console_addr: usize,
    /// Bellek Başlangıç Adresi
    pub ram_start: usize,
    /// Toplam Bellek Boyutu (bayt)
    pub ram_size: usize,
    /// Kesme Kontrolcüsü (GIC/PIC) adresi
...
impl DtbParser {
    ...
    pub fn parse_dtb(dtb_addr: usize) -> Result<HardwareConfig, KernelError> {
        ...
        // MIPS'te MMIO erişimi KSEG0/KSEG1 adresleri üzerinden yapılır.
        // DTB'den okunan adreslerin bu bölgelerde olup olmadığı kontrol edilmelidir.

        // Simülasyon: MIPS64 platformunda tipik donanım adresleri
        let config = HardwareConfig {
            // Seri Port (UART) adresi (Örn: 16550 Uyumlu UART)
            // KSEG1 bölgesinde temsili bir adres
            console_addr: 0xB000_03F8, 
            
            // Bellek bilgisi (genellikle 'memory' düğümünden alınır)
            // Fiziksel adres 0x0'dan başlar, ancak MIPS'te erişim 
            // 0x8000_0000 (KSEG0) veya 0xA000_0000 (KSEG1) üzerinden yapılır.
            ram_start: 0x0000_0000, 
            ram_size: 1024 * 1024 * 1024, // 1GB
            
            // Kesme Kontrolcüsü adresi (Temsili)
            interrupt_controller_addr: 0x8000_0000, 
        };
...
pub fn dump_dtb_header() {
    ...
    if dtb_addr == 0 {
        // serial_println! macro'sunun arch/mips64/console.rs'te tanımlandığını varsayıyoruz.
        serial_println!("[DTB-MIPS64] DTB adresi bilinmiyor (0x0).");
        return;
    }
    ...
    serial_println!("[DTB-MIPS64] DTB Adresi: {:#x}", dtb_addr);
    serial_println!("[DTB-MIPS64] Magic (Beklenen 0xd00dfeed): {:#x}", magic);
    
    // Ayrıştırılmış yapılandırma varsa onu da bas
    if let Ok(config) = DtbParser::get_config() {
        serial_println!("[DTB-MIPS64] -> Konsol Adresi: {:#x}", config.console_addr);
        serial_println!("[DTB-MIPS64] -> Kesme Kontrolcü Adresi: {:#x}", config.interrupt_controller_addr);
        serial_println!("[DTB-MIPS64] -> RAM Boyutu: {} MB", config.ram_size / 1024 / 1024);
    } else {
        serial_println!("[DTB-MIPS64] -> Yapılandırma henüz ayrıştırılmadı.");
    }
}