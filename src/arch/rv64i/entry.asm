# src/arch/rv64i/entry.s
# RISC-V 64 (RV64I) Mimarisi için Çekirdek Giriş Noktası Kodu.
...
# --------------------------------------------------------------------------------
_start:
    # Argümanlar: a0 ($x10), a1 ($x11)
    
    # Argümanları geçici olarak kalıcı yazmaçlara ($s0, $s1) kaydet.
    # a0 ($x10) -> s0 ($x8)
    # a1 ($x11) -> s1 ($x9)
    
    mv s0, a0                   # Argüman 1'i s0'a kopyala (DTB Adresi)
    mv s1, a1                   # Argüman 2'yi s1'e kopyala (Hart ID)

    # -----------------------------------
    # 2.1. Yazmaçları Temizleme
    # -----------------------------------
    
    # s0, s1 (Argümanlar) hariç diğerlerini sıfırla.
    ...
    # Argümanlar (a0, a1) temizlenecek çünkü s0, s1'de saklandı
    li a0, 0                    # $x10 (a0)
    li a1, 0                    # $x11 (a1)
    li a2, 0                    # $x12
    li a3, 0                    # $x13
