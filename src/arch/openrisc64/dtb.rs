...
pub struct HardwareConfig {
    ...
    pub ram_size: usize,
    /// OpenRISC Kesme Kontrolcüsü (ORPIC/PIC) adresi
...
impl DtbParser {
    ...
    pub fn parse_dtb(dtb_addr: usize) -> Result<HardwareConfig, KernelError> {
        ...
        // OpenRISC'te MMIO erişimi genellikle yüksek adreslerde yapılır.

        // Simülasyon: OpenRISC 64 platformunda tipik donanım adresleri
        let config = HardwareConfig {
            // Seri Port (UART) adresi (Örn: 16550 Uyumlu UART)
            console_addr: 0x9000_0000, // arch/openrisc64/console.rs'te kullanılan adres
            
            // Bellek bilgisi (genellikle 'memory' düğümünden alınır)
            ram_start: 0x0000_0000, 
            ram_size: 512 * 1024 * 1024, // 512MB
            
            // Kesme Kontrolcüsü (ORPIC) adresi (Temsili)
            interrupt_controller_addr: 0x9000_1000, 
        };
...
pub fn dump_dtb_header() {
    ...
    if dtb_addr == 0 {
        // serial_println! macro'sunun arch/openrisc64/console.rs'te tanımlandığını varsayıyoruz.
        serial_println!("[DTB-OR64] DTB adresi bilinmiyor (0x0).");
        return;
    }
    ...
    serial_println!("[DTB-OR64] DTB Adresi: {:#x}", dtb_addr);
    serial_println!("[DTB-OR64] Magic (Beklenen 0xd00dfeed): {:#x}", magic);
    
    // Ayrıştırılmış yapılandırma varsa onu da bas
    if let Ok(config) = DtbParser::get_config() {
        serial_println!("[DTB-OR64] -> Konsol Adresi: {:#x}", config.console_addr);
        serial_println!("[DTB-OR64] -> PIC Adresi: {:#x}", config.interrupt_controller_addr);
        serial_println!("[DTB-OR64] -> RAM Boyutu: {} MB", config.ram_size / 1024 / 1024);
    } else {
        serial_println!("[DTB-OR64] -> Yapılandırma henüz ayrıştırılmadı.");
    }
}